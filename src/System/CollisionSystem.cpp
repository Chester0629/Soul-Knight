...
void CollisionSystem::ResolveWall(glm::vec2& worldPos,
    ...
    const int rMin = std::max(worldToRow(hitCenter.y + hitHalf.y, rows) - 2, 0);
    const int rMax = std::min(worldToRow(hitCenter.y - hitHalf.y, rows) + 2, rows - 1);
    const int cMin = std::max(worldToCol(hitCenter.x - hitHalf.x, cols) - 2, 0);
    const int cMax = std::min(worldToCol(hitCenter.x + hitHalf.x, cols) + 2, cols - 1);
